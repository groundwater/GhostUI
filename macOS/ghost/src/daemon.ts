...
const server = Bun.serve({
  ...
  async fetch(req, server) {
    ...
    // Element screenshot: crop full screenshot to an element's AX frame
    // GET /api/element-screenshot?label=<label>&role=<role>
    // OR: /api/element-screenshot?fx=<x>&fy=<y>&fw=<w>&fh=<h>  (direct frame coords)
    if (req.method === "GET" && url.pathname === "/api/element-screenshot") {
      try {
        const label = url.searchParams.get("label");
        const role = url.searchParams.get("role");
        const id = url.searchParams.get("id");
        // Direct frame coordinates (logical, not pixel)
        const fx = url.searchParams.get("fx");
        const fy = url.searchParams.get("fy");
        const fw = url.searchParams.get("fw");
        const fh = url.searchParams.get("fh");
        const hasFrame = fx != null && fy != null && fw != null && fh != null;

        if (!label && !id && !hasFrame) {
          return new Response("Missing label/id or frame params (fx,fy,fw,fh)", { status: 400, headers: { "access-control-allow-origin": "*" } });
        }

        let elFrame: { x: number; y: number; width: number; height: number };
        let snap: ReturnType<typeof buildSnapshot>;

        if (hasFrame) {
          // Use provided frame coordinates directly
          elFrame = { x: Number(fx), y: Number(fy), width: Number(fw), height: Number(fh) };
          // Still need snap for scale factor detection (menu bar width)
          snap = buildSnapshot({ focusedDepth: 1 });
        } else {
          // Get AX snapshot to find element frame via N-API
          snap = buildSnapshot({ focusedDepth: 1000 });
          const axTree = snap?.channels?.focused?.items?.[0]?.tree;
          if (!axTree) {
            return new Response("No AX tree available", { status: 404, headers: { "access-control-allow-origin": "*" } });
          }

          // Find element by label/role (BFS) — match against label, title, or value
          type AXN = { role?: string; label?: string; title?: string; value?: string; frame?: { x: number; y: number; width: number; height: number }; children?: AXN[] };
          function findElement(node: AXN): AXN | null {
            if (label) {
              const matches = node.label === label || node.title === label || node.value === label;
              const matchRole = role ? node.role === role : true;
              if (matches && matchRole && node.frame) return node;
            }
          ...
          const el = findElement(axTree);
          if (!el || !el.frame) {
            return new Response(`Element not found: label=${label} role=${role}`, { status: 404, headers: { "access-control-allow-origin": "*" } });
          }
          elFrame = el.frame;
        }

        // Capture screenshot via N-API
        const screenshotData = screenshot("png");
        if (!screenshotData) {
          return new Response("Failed to capture screenshot", { status: 502, headers: { "access-control-allow-origin": "*" } });
        }
        const screenshotBuf = screenshotData.buffer;

        // Use sips to crop (macOS built-in) via a temp file
        const tmpIn = `/tmp/ghost-screenshot-full-${Date.now()}.png`;
        const tmpOut = `/tmp/ghost-screenshot-crop-${Date.now()}.png`;
        await Bun.write(tmpIn, screenshotBuf);

        // Detect scale factor: screenshot pixel width / logical screen width
        const sipsInfo = Bun.spawn(["sips", "-g", "pixelWidth", "-g", "pixelHeight", tmpIn], { stdout: "pipe", stderr: "pipe" });
        const sipsOut = await new Response(sipsInfo.stdout).text();
        await sipsInfo.exited;
        const pwMatch = sipsOut.match(/pixelWidth:\s*(\d+)/);
        const phMatch = sipsOut.match(/pixelHeight:\s*(\d+)/);
        const pixelW = pwMatch ? Number(pwMatch[1]) : 0;
        const pixelH = phMatch ? Number(phMatch[1]) : 0;

        // Get logical screen size from AX snapshot (menu bar width is a good proxy)
        const menuFrame = snap?.channels?.focused?.items?.[0]?.menuBar?.frame;
        const logicalW = menuFrame?.width || (pixelW / 2);
        const scale = pixelW / logicalW;

        const f = elFrame;
        const sx = Math.round(f.x * scale);
        const sy = Math.round(f.y * scale);
        ...
        const cw = Math.min(sw, pixelW - cx);
        const ch = Math.min(sh, pixelH - cy);

        // sips crop is: --cropToHeightWidth <h> <w> --cropOffset <y> <x>
        const cropProc = Bun.spawn([
          "sips", tmpIn,
          "--cropToHeightWidth", String(ch), String(cw),
          "--cropOffset", String(cy), String(cx),
          "--out", tmpOut,
        ], { stdout: "pipe", stderr: "pipe" });
        await cropProc.exited;

        const cropped = await Bun.file(tmpOut).arrayBuffer();
        // Clean up temp files
        try { await Bun.spawn(["rm", tmpIn, tmpOut]).exited; } catch {}

        // Fire flash animation on the captured element
        try {
          if (nativeAX?.axPostOverlay) {
            nativeAX.axPostOverlay("flash", JSON.stringify({ rect: elFrame }));
          }
        } catch {}

        return new Response(cropped, {
          headers: { "content-type": "image/png", "access-control-allow-origin": "*", "cache-control": "no-cache" },
        });
      } catch (e: unknown) {
        return new Response(errorMessage(e) || "screenshot failed", { status: 502, headers: { "access-control-allow-origin": "*" } });
      }
    }
