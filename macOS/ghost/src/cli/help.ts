...
const HELP_TOPICS: HelpTopic[] = [
  {
    ...
    related: ["query-language", "output", "ids"],
  },
  {
    id: "focused",
    title: "Return the focused AX target",
    summary: "Emit one pipeable AX target-bearing payload for the frontmost focused element.",
    aliases: [],
    usage: ["gui focused"],
    examples: [
      "gui focused",
      "gui focused | gui click -",
      "gui focused | gui type - 'Ada'",
    ],
    notes: [
      "Prints one JSON CLI payload that carries an AX target for the current frontmost focused element.",
      "Fails when the focused AX query does not resolve to exactly one target-bearing match.",
      "Use `gui cursor` when you specifically need text selection / caret metadata.",
    ],
    related: ["cursor", "click", "type", "ax query"],
  },
  {
    id: "cursor",
    title: "Return the active text cursor payload",
    summary: "Emit the existing AX cursor payload for the currently focused text-editable element.",
    aliases: [],
    usage: ["gui cursor"],
    examples: [
      "gui cursor",
      "gui cursor | gui type - 'foo'",
    ],
    notes: [
      "This is the intent-first top-level alias for `gui ax cursor`.",
      "Returns an `ax.cursor` JSON payload with target plus selection or caret metadata when available.",
      "Fails when no focused text cursor is currently available.",
    ],
    related: ["focused", "type", "ax cursor"],
  },
  {
    id: "click",
    title: "Click one resolved target",
    summary: "Consume one AX target-bearing payload from stdin or a literal JSON payload and click it.",
    aliases: [],
    usage: ["gui click -", "gui click '<json>'"],
    examples: [
      "gui focused | gui click -",
      "gui ax query --only 'Button#Save' | gui click -",
    ],
    notes: [
      "Consumes exactly one AX target-bearing JSON CLI payload from stdin or a literal JSON payload.",
      AX_TARGET_JSON_TYPE_NOTE,
      "Routes through the daemon semantic action path first and falls back to the raw AX click path only when the target cannot be semantically routed.",
    ],
    related: ["focused", "query", "type", "ax click"],
  },
  {
    id: "scroll",
    title: "Scroll at one resolved target",
    summary: "Consume one AX target-bearing payload from stdin or a literal JSON payload and inject a scroll event at that point.",
    aliases: [],
    usage: ["gui scroll - --dx <n> --dy <n>", "gui scroll '<json>' --dx <n> --dy <n>"],
    examples: [
      "gui focused | gui scroll - --dx 0 --dy -240",
      "gui ax query --only 'ScrollArea' | gui scroll - --dx 0 --dy -240",
    ],
    notes: [
      "Consumes exactly one AX target-bearing JSON CLI payload from stdin or a literal JSON payload.",
      AX_TARGET_JSON_TYPE_NOTE,
      "Negative --dy scrolls down; positive --dy scrolls up.",
      "Routes through the daemon semantic action path first and falls back to the raw CG scroll injection path only when semantic routing cannot own the target.",
    ],
    related: ["focused", "query", "cg scroll"],
  },
  {
    id: "shortcut",
    title: "Send a keyboard shortcut",
    summary: "Send a key combo, special key, or text string globally, or focus one resolved target first and then send it.",
    aliases: [],
    usage: ["gui shortcut <combo|key|text>", "gui shortcut - <combo|key|text>", "gui shortcut '<json>' <combo|key|text>"],
    examples: [
      "gui shortcut cmd+n",
      "gui shortcut return",
      "gui shortcut 'Hello World'",
      "gui focused | gui shortcut - cmd+a",
    ],
    notes: [
      "With a target-bearing payload first, this focuses that target through the daemon semantic action path before sending the keyboard input.",
      "Combos use `+` separators, for example `cmd+shift+p`.",
      "A combo with only modifiers is invalid.",
    ],
    related: ["type", "cg key"],
  },
  {
    id: "type",
    title: "Type text into one resolved target",
    summary: "Consume one AX cursor or other AX target-bearing payload from stdin or a literal JSON payload and type text.",
    aliases: [],
    usage: ["gui type - '<value>'", "gui type '<json>' '<value>'"],
    examples: [
      "gui cursor | gui type - 'foo'",
      "gui ax query --only 'TextField#Name' | gui type - 'Ada'",
    ],
    notes: [
      "Consumes exactly one AX cursor or other AX target-bearing JSON CLI payload from stdin or a literal JSON payload.",
      AX_TARGET_BEARING_JSON_TYPE_NOTE,
      "Routes through the daemon semantic action path first.",
      "When passed an `ax.cursor`, it falls back to the cursor-aware AX type path if semantic routing cannot own the target.",
      "When passed a plain AX target, it falls back to the existing AX type behavior if semantic routing cannot own the target.",
    ],
    related: ["cursor", "focused", "shortcut", "ax type"],
  },
  {
    id: "ca",
  ...
  },
  {
    ...
    examples: [
      ...
      "gui vat query 'Window[frame]' | gui gfx scan --duration 750 --direction left-to-right -",
      "gui vat query 'Window[frame]' | gui gfx xray --direction bottom-to-top -",
  ...
  },
  {
    ...
    related: ["window focus", "window drag", "cg windows"],
  },
  {
    id: "window focus",
  ...
  },
  {
    ...
    aliases: [],
    usage: [
      "gui actor spawn pointer <name> [--duration-scale <scale>] [--idle <ms>]",
      "gui actor spawn canvas <name>",
      "gui actor spawn spotlight <name>",
      "gui actor run <name>.<action> ...",
      "gui actor kill <name | ->",
      "gui actor list",
    ],
    examples: [
      "gui actor spawn pointer pointer.main --idle 2500",
      "gui actor spawn canvas canvas.notes",
      "gui actor spawn spotlight spotlight.focus",
      ...
      "gui actor run spotlight.focus.rect --padding 12 --blur 16 -",
      "gui actor run canvas.notes.draw rect",
      "gui actor kill pointer.main",
      "gui actor list | gui actor kill -",
    ],
    notes: [
      ...
      "Pointer actors default to an idle pulse every 3000ms; pass --idle 0 to disable it.",
      "Canvas actors retain marker and text items until gui actor run <name>.clear or gui actor kill <name>.",
      "gui actor kill - reads actor names or actor-list JSON from stdin, so gui actor list can be piped straight into it.",
  ...
  },
  {
    ...
    aliases: [],
    usage: ["gui actor spawn pointer <name> [--duration-scale <scale>] [--idle <ms>]", "gui actor spawn canvas <name>", "gui actor spawn spotlight <name>"],
    examples: [
      "gui actor spawn pointer pointer.main --idle 2500",
      "gui actor spawn canvas canvas.notes",
      "gui actor spawn spotlight spotlight.focus",
      "gui actor spawn pointer pointer.main --duration-scale 0 --idle 0",
    ],
  ...
  },
  {
    ...
    related: ["actor spawn", "actor spawn spotlight", "actor run draw", "actor run text", "actor run clear"],
  },
  {
    id: "actor spawn spotlight",
    title: "Spawn a spotlight actor",
    summary: "Create a named persistent spotlight actor instance.",
    aliases: [],
    usage: ["gui actor spawn spotlight <name>"],
    examples: [
      "gui actor spawn spotlight spotlight.focus",
    ],
    notes: [
      "Spotlight actors retain their latest rect/circ geometry until they are turned off or killed.",
    ],
    related: ["actor spawn", "actor run rect", "actor run circ", "actor run on", "actor run off", "actor run color"],
  },
  {
    id: "actor list",
  ...
  },
  {
    ...
    examples: ["gui actor kill pointer.main", "gui actor list | gui actor kill -"],
    notes: [
      "Pass - to read actor names or actor-list JSON from stdin.",
      "Each stdin line may also be a raw actor name.",
    ],
  ...
  },
  {
    ...
    usage: [
      ...
      "gui actor run <name>.encircle [--at <x> <y> | -] [--radius <px>] [--speed <px/s>] [--loops <n>] [--timeout <ms>]",
      "gui actor run <name>.rect [--padding <pixels>] [--blur <pixels>] [--speed <px/s>] -",
      "gui actor run <name>.circ [--padding <pixels>] [--blur <pixels>] [--speed <px/s>] -",
    ...
    examples: [
      ...
      "gui actor run pointer.main.narrate --text \"Opening Settings\"",
      "gui actor run pointer.main.encircle --at 840 420 --radius 72 --speed 420 --loops 2",
      "gui actor run spotlight.focus.rect --padding 12 --blur 16 --speed 240 -",
      "gui actor run spotlight.focus.circ --padding 8 --blur 24 --speed 180 -",
      "gui actor run spotlight.focus.on --transition instant",
      "gui actor run spotlight.focus.off --transition fade",
      "gui actor run spotlight.focus.color 'rgba(0,0,0,0.35)'",
    ...
    notes: [
      ...
      "A newer run on the same actor preempts the older in-flight action.",
      "Pointer click and encircle actions move visibly into place first when they need to reposition the actor.",
      "Spotlight rect and circ calls retain geometry on the same actor id and animate successive size and position changes.",
      "Canvas draw and text actions persist until gui actor run <name>.clear or gui actor kill <name>.",
      "Bare gui actor run <name> resolves the actor type when possible and prints type-appropriate usage.",
    ],
    related: ["actor", "actor run move", "actor run click", "actor run drag", "actor run scroll", "actor run think", "actor run narrate", "actor run encircle", "actor run rect", "actor run circ", "actor run on", "actor run off", "actor run color", "actor run draw", "actor run text", "actor run clear", "actor run dismiss"],
  },
  {
    id: "actor run draw",
  ...
  {
    id: "actor run rect",
    title: "Run spotlight rect",
    summary: "Retain a rectangular spotlight cutout on the named spotlight actor.",
    aliases: [],
    usage: ["gui actor run <name>.rect [--padding <pixels>] [--blur <pixels>] [--speed <px/s>] -"],
    examples: [
      "gui actor run spotlight.focus.rect --padding 12 --blur 16 -",
      "gui ax query --only 'Window[frame]' | gui actor run spotlight.focus.rect --padding 8 --blur 20 -",
    ],
    notes: [
      "Pass `-` to read one AX/VAT target-bearing payload from stdin and use every resolved bounds rectangle.",
      "Repeated rect calls on the same spotlight actor animate geometry changes in place.",
      "Use --speed <px/s> to keep spotlight motion at a constant physical-pixel speed across different distances.",
      "Use --padding to inflate every resolved rect before it is rendered.",
      "Use --blur to soften the spotlight edge.",
    ],
    related: ["actor run", "actor spawn spotlight", "actor run circ", "actor run on", "actor run off", "actor run color"],
  },
  {
    id: "actor run circ",
    title: "Run spotlight circ",
    summary: "Retain a circular spotlight cutout on the named spotlight actor.",
    aliases: [],
    usage: ["gui actor run <name>.circ [--padding <pixels>] [--blur <pixels>] [--speed <px/s>] -"],
    examples: [
      "gui actor run spotlight.focus.circ --padding 12 --blur 18 -",
      "gui ax query --only 'Window[frame]' | gui actor run spotlight.focus.circ --padding 8 --blur 20 -",
    ],
    notes: [
      "Pass `-` to read one AX/VAT target-bearing payload from stdin and use every resolved bounds rectangle.",
      "Repeated circ calls on the same spotlight actor animate geometry changes in place.",
      "Use --speed <px/s> to keep spotlight motion at a constant physical-pixel speed across different distances.",
      "Use --padding to inflate every resolved rect before it is rendered.",
      "Use --blur to soften the spotlight edge.",
    ],
    related: ["actor run", "actor spawn spotlight", "actor run rect", "actor run on", "actor run off", "actor run color"],
  },
  {
    id: "actor run on",
    title: "Turn on a spotlight actor",
    summary: "Reveal the retained spotlight geometry on the named spotlight actor.",
    aliases: [],
    usage: ["gui actor run <name>.on [--transition fade|instant]"],
    examples: [
      "gui actor run spotlight.focus.on",
      "gui actor run spotlight.focus.on --transition instant",
    ],
    notes: [
      "The spotlight stays attached to the actor's retained geometry.",
      "Use fade to animate the opacity back in, or instant to snap it on.",
    ],
    related: ["actor run", "actor spawn spotlight", "actor run rect", "actor run circ", "actor run off", "actor run color"],
  },
  {
    id: "actor run off",
    title: "Turn off a spotlight actor",
    summary: "Hide the retained spotlight geometry on the named spotlight actor.",
    aliases: [],
    usage: ["gui actor run <name>.off [--transition fade|instant]"],
    examples: [
      "gui actor run spotlight.focus.off",
      "gui actor run spotlight.focus.off --transition instant",
    ],
    notes: [
      "The spotlight geometry stays retained on the actor and can be turned back on.",
      "Use fade to animate opacity to zero, or instant to snap it off.",
    ],
    related: ["actor run", "actor spawn spotlight", "actor run rect", "actor run circ", "actor run on", "actor run color"],
  },
  {
    id: "actor run color",
    title: "Change a spotlight color",
    summary: "Update the fill color retained by the named spotlight actor.",
    aliases: [],
    usage: ["gui actor run <name>.color <Color>"],
    examples: [
      "gui actor run spotlight.focus.color 'rgba(0,0,0,0.35)'",
      "gui actor run spotlight.focus.color '#101010C0'",
    ],
    notes: [
      "Color updates reuse the retained spotlight geometry and only change the fill color.",
    ],
    related: ["actor run", "actor spawn spotlight", "actor run rect", "actor run circ", "actor run on", "actor run off"],
  },
  {
    id: "actor run text",
  ...
  {
    id: "actor run encircle",
    title: "Run encircle",
    summary: "Orbit a pointer actor around a target area to draw attention.",
    aliases: [],
    usage: ["gui actor run <name>.encircle [--at <x> <y> | -] [--radius <px>] [--speed <px/s>] [--loops <n>] [--timeout <ms>]"],
    examples: [
      "gui actor run pointer.main.encircle --at 840 420 --radius 72 --speed 420 --loops 2",
      "gui ax query --only 'Button#Save' | gui actor run pointer.main.encircle - --loops 1",
    ],
    notes: [
      "Pass `-` to read one AX/VAT target-bearing payload from stdin and orbit around the resolved bounds center.",
      "When radius is omitted in stdin mode, a sensible default is derived from the resolved bounds size.",
    ],
    related: ["actor run"],
  },
  {
  ...
  },
  {
    ...
    examples: [
      ...
      "gui vat mount /System/Dock dock",
    ...
    notes: [
      ...
      "The dock driver snapshots com.apple.dock and flattens Dock items into AppIcon, Stack, MinimizedWindow, Separator, and Trash nodes.",
  ...
  },
  {
    ...
    notes: [
      ...
      "Use gui query when you want GhostUI's normalized document instead of raw AX.",
    ],
  ...
  },
  {
    ...
    examples: [
      "GUIML on stdout for gui query",
      "JSON on stdout for gui cg windows and gui ws frontmost",
    ...
    related: ["query", "rec", "ax", "cg", "ws", "log"],
  },
  {
    id: "ids",
    title: "Node ids",
...
export function renderRootHelp(): string {
  return [
    ...
    "",
    formatSection("Intent-first commands", [
      "gui query '<query>'         Query the live UI tree (alias: gui q)",
      "gui focused                Emit the focused AX target payload",
      "gui cursor                 Emit the active text cursor payload",
      "gui click -                Click one piped AX target payload",
      "gui scroll - --dx --dy     Scroll at one piped AX target payload",
      "gui shortcut <combo>       Send a keyboard shortcut or key/text input",
      "gui type - '<value>'       Type into one piped AX cursor/target payload",
      "gui rec <mode>             Capture a rect or cgWindowId",
    ]),
    "",
    formatSection("Visual and runtime tools", [
    ...
    "",
    formatSection("Advanced raw commands", [
