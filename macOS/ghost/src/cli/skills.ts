...
function renderSkillBody(target: SkillTarget): string {
  ...
  return [
    ...
    "",
    "## Command map",
    `- Live GUI state: \`${usageLine("query")}\``,
    ...
    `- System state: \`${usageLine("log")}\``,
    "",
    "## Operating rules",
    "- Prefer `gui query` over raw AX unless you specifically need raw AX roles, values, or actions.",
    ...
    `- For daemon-backed changes, confirm with the matching read command or query route instead of assuming success.`,
