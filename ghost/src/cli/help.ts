...
const DRAW_SCRIPT_JSON_TYPE_NOTE = [
  ...
  "    `DrawScript = { coordinateSpace: \"screen\"; timeout?: number; items: Array<{ kind: \"rect\" | \"line\" | \"xray\"; ... }> }`",
].join("\n");
...
const HELP_TOPICS: HelpTopic[] = [
  ...
  },
  {
    ...
    notes: [
      ...
      "Supports rect, line, and xray items in screen coordinates.",
    ],
    related: ["output"],
  },
  {
    ...
    aliases: [],
    usage: [
      "gui gfx outline [--timeout <ms>] -",
      "gui gfx scan [--timeout <ms>] -",
      "gui gfx xray [--timeout <ms>] -",
      "gui gfx spotlight [--timeout <ms>] -",
      "gui gfx arrow [--timeout <ms>] -",
      "gui gfx text [--timeout <ms>] \"<text>\" -",
    ],
    examples: [
      "gui ax query --only --app Terminal '@@{Button[subrole~=DecrementPage]}' | gui gfx outline -",
      "gui vat query 'Window[frame]' | gui gfx scan --timeout 750 -",
      "gui vat query 'Window[frame]' | gui gfx xray -",
      "gui vat query 'Window[frame]' | gui gfx spotlight -",
      "gui vat query 'Window[frame]' | gui gfx arrow -",
      "gui vat query 'Window[frame]' | gui gfx text \"Review this\" -",
    ],
    notes: [
